    ...
    async def lastperson07_handle_broadcast(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        try:
            ...
            if broadcast_type == "group":
                results = await lastperson07_broadcaster.lastperson07_broadcast_to_groups(message)
                ...
                if tier == "free":
                    results = await lastperson07_broadcaster.lastperson07_broadcast_to_free_users(message)
                elif tier == "premium":
                    results = await lastperson07_broadcaster.lastperson07_broadcast_to_premium_users(message)
                else:
                    results = await lastperson07_broadcaster.lastperson07_broadcast_to_users(message)
